<?php


return [

    "login" => [
        "heading" => "Login into your account",
        "title" => "Login into your account",
        "email_address" => "Email address",
        "enter_email" => "Enter your email",
        "name_email" => "Email or User Name",
        "enter_email_name" => "Enter your email or username",
        "phone_number" => "Phone Number",
        "enter_phone_number" => "Enter your phone number",
        "signin_phone_number" => "Sign in with Phone Number ?",
        "signin_email" => "Sign in with Email or Username ?",
        "email_privacy_info" => "We'll never share your email with anyone else.",
        "password" => "Password",
        "enter_password" => "Enter Your Passwowrd",
        "forgot_password" => "Forgot Password?",
        "login" => "Login",
        "logout" => "Logout",
        "signup" => "Need an account? Sign up!"
    ],

    "register" => [
        "title" => "Create a new account",
        "new_account" => "Create a new account",
        "continue_app" => "to continue the app",
        "name" => "Name",
        "enter_name" => "Enter your name",
        "confirm_password" => "Confirm Password",
        "enter_confirm_password"=>"Enter confirm password",
        // "f_name" => "First Name",
        // "l_name" => "Last Name",
        // "enter_first_name" => "Enter your first name",
        // "enter_last_name" => "Enter your last name",
        "email_address" => "Email address",
        "enter_email" => "Enter your email",
        "email_privacy_info" => "We'll never share your email with anyone else.",
        "phone_number" => "Phone Number",
        "enter_phone_number" => "Enter your phone number",
        "password" => "Password",
        "enter_password" => "Enter Your Passwowrd",
        "signin" => "Sign in instead",
        "register" => "register"
    ],

    "forget-password" => [
        "title" => "Reset password",
        "reset_password" => "Reset Password",
        "email_address" => "Email Address",
        "enter_email" => "Enter email",
        "send_email" => "We'll send an verification link into your email",
        "return_login" => "Return to login",
        "signup" => "Need an account? Sign up!"
    ],
    "dashboard" => [
        "heading" => "Welcome to dashboard",
    ]


];
